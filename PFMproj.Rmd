...
######Attempt 1 - Treat each timestamp measurement as a sample, each sensor as a variable. pfmc1.1 is the first dataset of pfmcdata. 

```{r}
pfmc1.1 = pfmcdatalist[[1]][[2]]
# define rownames
rownames(pfmc1.1) =  1:nrow(pfmc1.1)
pfmc1.1pca = prcomp(pfmc1.1[-9], center = TRUE, scale. = TRUE)
```
...
```{r}
pfmc1.1pca.var = pfmc1.1pca$sdev^2
# In percentage
pfmc1.1pca.var.per = round(pfmc1.1pca.var / sum(pca.var) * 100, 1)
# Plot the percentage
barplot(pfmc1.1pca.var.per, names.arg = paste0("PC", 1:length(pfmc1.1pca.var)),
          main = "Scree Plot", 
        xlab = "Principal Component", ylab = "Percent Variation")
text(c(0.8, 1.8), pfmc1.1pca.var.per[1:2]-1, pfmc1.1pca.var.per[1:2])
```
...
loading_scores = pfmc1.1pca$rotation[ , 1] 
...
top_8_variables = names(variable_scores_ranked)
pfmc1.1pca$rotation[top_8_variables, 1]
```
###### Attempt 1.5 - select a random sample from sample1 then plot.
```{r}
...
###### Attempt 2 - Treat each sensor as a sample(row), each timestamp as a variable. Sample1 is the first dataset of pfmcdata. 

```{r}
# Tidy data
pfmc1.5 = pfmcdatalist[[1]][[2]]

pfmc1.5 = pfmc1.5 %>% tidy()
rownames(pfmc1.5) = 1:nrow(pfmc1.5)

pfmc1.5.pca = prcomp(pfmc1.5[-2], center = TRUE, scale. = TRUE)
```


```{r}
autoplot(pfmc1.5.pca, data = pfmc1.5, colour = "sensor")
```


```{r}
ggbiplot(pfmc1.5.pca)
```

```{r}
ggbiplot(pfmc1.5.pca, ellipse = TRUE, groups = pfmc1.5$sensor)
