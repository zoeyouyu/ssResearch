...
###### ------------------Lets explore PCA ------------------------ ######
Treat each timestamp as samples, each sensor as variable
sample1 is the first dataset of pfmcdata. 

```{r}
sample1 = pfmcdatalist[[1]][[2]]
# define rownames
rownames(sample1) = as.character((sample1$rectime - min(sample1$rectime)) / 10)
pca = prcomp(sample1[-9], center = TRUE, scale. = TRUE)

pca.var = pca$sdev^2
# In percentage
pca.var.per = round(pca.var / sum(pca.var) * 100, 1)
# Plot the percentage
barplot(pca.var.per, names.arg = paste0("PC", 1:8),
          main = "Scree Plot", 
        xlab = "Principal Component", ylab = "Percent Variation")
text(c(0.8, 1.8), pca.var.per[1:2]-1, pca.var.per[1:2])
```
...
ggplot() +
  geom_point(data = pca.data, aes(x = X, y = Y)) + 
  xlab(paste("PC1 - ", pca.var.per[1], "%", sep = "")) +
  ylab(paste("PC2 - ", pca.var.per[2], "%", sep = "")) +
  ggtitle("PCA Graph for Sample1")


# Try color coding each 1000 ms sample
```
...
pca$rotation[top_8_variables, 1]
```
