...
```{r}
myplot(df.list[[2]][1:(end-50), ], name = names(df.list)[2]) + theme_classic()
# temp
plot.temp(df.list[[2]][1:(end-50), ], name = names(df.list)[2]) + theme_classic()
```
...
# temp
plot.temp(df.list[[4]], name = names(df.list)[4]) + theme_classic()
```
...
### Greedy method ###
#### Trying to get  `n_peaks*2` change points at once using Binary Segementation, manually choose Q = 10
...
### Dropping the starting (0.5s) and ending (0.5s) baseline data for each contraction
```{r}
contraction.data = list()
for (i in 1:n_PFMC) {
    contraction.data[[i]] = getsubdata(change, raw.start.end.rectime[i*2 - 1] + 1000, raw.start.end.rectime[i*2] - 1000)
