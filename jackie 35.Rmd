...
plot.temp(first3seconds, name = "first 3 seconds")

myplot(first3seconds, name = "first 3 seconds")
...
```{r}
pressure.plot = change %>% filter(rectime < (the_knack_start - squeeze_and_hold_start - 19000)) %>%
  myplot(name = "Squeeze and hold") + theme_classic()

temp.plot = whole %>% filter(rectime < (the_knack_start - squeeze_and_hold_start - 19000)) %>%
...
ggarrange(pressure.plot + rremove("xlab"), 
                   temp.plot,
                    ncol = 1, nrow = 2, align = "v", 
                   common.legend = TRUE, legend = "right")
#ggsave("Temp Jackie 35.png", width = 10, height = 5)
```
...
## Temperature - Did the femfit move?
