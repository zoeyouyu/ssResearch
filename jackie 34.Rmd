...
### A helper function that define the column names
```{r}
...
p = myplot(whole, name = "the whole recording")

### Check temperature

t = plot.temp(whole, name = "the whole recording")
...
pressure.plot = myplot(change, name = "the whole recording (baseline removed)") + theme_classic()


temp.plot = plot.temp(whole, name = "the whole recording (baseline removed)") + theme_classic()

ggarrange(pressure.plot + rremove("xlab"), 
                   temp.plot,
                    ncol = 1, nrow = 2, align = "v", 
                   common.legend = TRUE, legend = "right")
```
...
```{r}
pressure.plot = change %>% filter(rectime < (the_knack_start - squeeze_and_hold_start - 75000)) %>%
  myplot(name = "Squeeze and hold") + theme_classic()

temp.plot = whole %>% filter(rectime < (the_knack_start - squeeze_and_hold_start - 75000)) %>%
...
### Dropping the starting (1s) and ending (1s) baseline data fro each contraction
```{r}
contraction.data = list()
for (i in 1:20) {
    contraction.data[[i]] = getsubdata(change, raw.start.end.rectime[i*2 - 1] + 1000, raw.start.end.rectime[i*2] - 00)
...
ggplot(data = avged.df, aes(x = contraction, y = mean, group = sensor, color = sensor)) +
  ...
  labs(title = "Mean Pressure over contraction", 
       y = "Mean pressure (mmHg)", 
       x = "Contraction",
       subtitle = "Jackie session 34") +
  theme_classic() +
  scale_color_brewer(palette = "Set1")
